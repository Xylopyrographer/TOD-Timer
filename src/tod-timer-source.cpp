...
// --------------------------------------------------------------------------
static std::string build_timer_string(long long remaining_ms, SegmentFormat fmt_h, SegmentFormat fmt_m,
				      SegmentFormat fmt_s, SegmentFormat fmt_t)
{
	...
	auto write_seg = [&](long long val, SegmentFormat fmt, bool is_tenths) {
		...
		const bool conditional =
			(fmt == SegmentFormat::IF_NONZERO_NO_LEAD || fmt == SegmentFormat::IF_NONZERO_LEAD);
		...
		const bool explicit_lead = (fmt == SegmentFormat::ALWAYS_LEAD || fmt == SegmentFormat::IF_NONZERO_LEAD);
...
static std::string build_timer_string_from_source(long long remaining_ms, const TodTimerSource *d)
{
	return build_timer_string(remaining_ms,
...
static void apply_appearance(TodTimerSource *d, obs_data_t *settings)
{
	...
	// Shadow
	obs_data_set_bool(ts, "drop_shadow", obs_data_get_bool(settings, S_DROP_SHADOW));
	...
	obs_data_set_bool(ts, "outline", obs_data_get_bool(settings, S_OUTLINE));
...
static void *tod_create(obs_data_t *settings, obs_source_t *source)
{
	...
	d->source = source;
	d->text_source = obs_source_create_private(TEXT_SOURCE_ID, "tod_timer_inner_text", nullptr);
...
static bool tod_on_configure(obs_properties_t *, obs_property_t *, void *data)
{
	...
	SettingsDialog dlg(settings, static_cast<QWidget *>(obs_frontend_get_main_window()));
...
static obs_properties_t *tod_get_properties(void *data)
{
	...
	snprintf(s_ver_buf, sizeof(s_ver_buf), "v%s (build %s)", PLUGIN_VERSION, PLUGIN_BUILD_NUMBER);
	...
	obs_properties_add_button(props, "configure", obs_module_text("Configure"), tod_on_configure);
	return props;
#else
	(void)data;
	obs_properties_t *props = obs_properties_create();
	obs_properties_add_text(props, "version", s_ver_buf, OBS_TEXT_INFO);
	obs_properties_add_int(props, S_TARGET_HOUR, obs_module_text("TargetHour"), 0, 23, 1);
	obs_properties_add_int(props, S_TARGET_MINUTE, obs_module_text("TargetMinute"), 0, 59, 1);
	obs_properties_add_int(props, S_TARGET_SECOND, obs_module_text("TargetSecond"), 0, 59, 1);
	obs_properties_add_int(props, S_TARGET_TENTHS, obs_module_text("TargetTenths"), 0, 9, 1);
	...
	auto add_fmt = [&](const char *key, const char *label) {
		obs_property_t *p =
			obs_properties_add_list(props, key, label, OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
		obs_property_list_add_int(p, obs_module_text("FmtAlwaysNoLead"), (int)SegmentFormat::ALWAYS_NO_LEAD);
		obs_property_list_add_int(p, obs_module_text("FmtAlwaysLead"), (int)SegmentFormat::ALWAYS_LEAD);
		obs_property_list_add_int(p, obs_module_text("FmtIfNonzeroNoLead"),
					  (int)SegmentFormat::IF_NONZERO_NO_LEAD);
		obs_property_list_add_int(p, obs_module_text("FmtIfNonzeroLead"), (int)SegmentFormat::IF_NONZERO_LEAD);
		obs_property_list_add_int(p, obs_module_text("FmtHideRolldown"), (int)SegmentFormat::HIDE_ROLLDOWN);
	};
...
static void tod_video_tick(void *data, float /*seconds*/)
{
	...
	if (!d->running.load(std::memory_order_relaxed)) {
		...
		if (d->last_display.empty()) {
			const long long placeholder_ms = (long long)d->target_hour.load() * 3600000LL +
		...
		const long long target_ms =
			(long long)d->target_hour.load() * 3600000LL + (long long)d->target_minute.load() * 60000LL +
			(long long)d->target_second.load() * 1000LL + (long long)d->target_tenths.load() * 100LL;

		// Current time expressed as milliseconds since midnight.
		const long long current_ms =
			((long long)lt.tm_hour * 3600LL + (long long)lt.tm_min * 60LL + (long long)lt.tm_sec) * 1000LL +
