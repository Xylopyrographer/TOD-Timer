...
{
	CFStringRef familyCF = CFStringCreateWithCharacters(kCFAllocatorDefault,
							    reinterpret_cast<const UniChar *>(family.constData()),
							    static_cast<CFIndex>(family.size()));
	CFDictionaryRef attrs = CFDictionaryCreate(kCFAllocatorDefault, (const void **)&kCTFontFamilyNameAttribute,
						   (const void **)&familyCF, 1, &kCFTypeDictionaryKeyCallBacks,
...
SettingsDialog::SettingsDialog(obs_data_t *settings, QWidget *parent) : QDialog(parent)
{
...
void SettingsDialog::buildUi()
{
	...
	m_timeEdit->setToolTip(obs_module_text("TargetTimeToolTip"));
	...
	connect(m_fontFamily, &QFontComboBox::currentFontChanged, this, &SettingsDialog::onFontFamilyChanged);
...
void SettingsDialog::populateFromSettings(obs_data_t *settings)
{
	...
	m_timeEdit->setText(
		QString("%1:%2:%3.%4").arg(displayH).arg(min, 2, 10, QChar('0')).arg(sec, 2, 10, QChar('0')).arg(tth));
	...
	m_color = QColor((int)((abgr >> 0) & 0xFF),   // R
			 (int)((abgr >> 8) & 0xFF),   // G
			 (int)((abgr >> 16) & 0xFF),  // B
			 (int)((abgr >> 24) & 0xFF)); // A
	updateColorButton();
...
void SettingsDialog::updateColorButton()
{
	...
	m_colorButton->setStyleSheet(QString("background-color: rgba(%1,%2,%3,%4); border: 1px solid #888;")
					     .arg(m_color.red())
					     .arg(m_color.green())
					     .arg(m_color.blue())
					     .arg(m_color.alpha()));
...
{
	QColor c =
		QColorDialog::getColor(m_color, this, obs_module_text("ChooseColor"), QColorDialog::ShowAlphaChannel);
	if (c.isValid()) {
		m_color = c;
		updateColorButton();
	}
...
void SettingsDialog::parseTimeInput(int &hour, int &min, int &sec, int &tenths) const
{
	...
	hour = isPm ? (displayH == 12 ? 12 : displayH + 12) : (displayH == 12 ? 0 : displayH);
...
void SettingsDialog::applyToSettings(obs_data_t *settings) const
{
	...
	int flags = 0;
	if (styleLower.contains("bold") || styleLower.contains("heavy") || styleLower.contains("black") ||
	    styleLower.contains("semibold") || styleLower.contains("demibold") || styleLower.contains("medium")) {
		flags |= 1;
	}
	...
	const uint32_t abgr = ((uint32_t)m_color.alpha() << 24) | ((uint32_t)m_color.blue() << 16) |
			      ((uint32_t)m_color.green() << 8) | ((uint32_t)m_color.red() << 0);
